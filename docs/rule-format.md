...
- `schemaVersion` — an integer, currently `1`. A future breaking change to the `rule` shape bumps this; `RuleJsonCodec.decode` rejects any `schemaVersion` newer than the app understands with a clear error, rather than silently misinterpreting it.
...
```json
{
  ...
  "rule": {
    ...
    "actions": [
      { "id": "a1", "type": "save_data", "isEnabled": true, "config": {} }
    ],
