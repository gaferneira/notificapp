# Delta for rule-storage

No prior `openspec/specs/rule-storage/spec.md` exists — condition persistence was previously documented only in ADR 011 prose, not as a Gherkin capability spec. This delta introduces the capability's first formal requirements, scoped to the condition-storage change; it does not restate field/action/rule persistence, which stay on the pre-existing normalized-table shape (ADR 011 unchanged for those).
