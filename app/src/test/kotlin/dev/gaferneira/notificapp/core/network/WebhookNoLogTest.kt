...
/**
 * Automated no-log guarantee test (design.md): exercises [WebhookRepositoryImpl] and
 * [WebhookTestClient] failure paths (dao exception, malformed header value, network error) with a
 * fixed sample `authValue` and asserts it never appears in any Timber-logged message, throwable
 * message, or throwable `toString()`.
 */
