...
class FakeWebhookRepository(initial: List<Webhook> = emptyList()) : WebhookRepository {
    ...
    override suspend fun updateDeliveryStatus(id: String, status: WebhookDeliveryStatus, at: Long): Result<Unit> {
        webhooks.value = webhooks.value.map { webhook ->
            if (webhook.id == id) webhook.copy(lastDeliveryStatus = status, lastDeliveryAt = at) else webhook
        }
        return Result.success(Unit)
    }
}
