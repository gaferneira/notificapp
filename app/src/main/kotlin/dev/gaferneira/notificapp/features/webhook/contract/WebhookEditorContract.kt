...
object WebhookEditorContract {
    ...
    /** HTTP method choice surfaced in the UI, mirroring [dev.gaferneira.notificapp.domain.model.HttpMethod]. */
    enum class HttpMethodUi { GET, POST, PUT, PATCH, DELETE }
    ...
    data class UiState(
        ...
        val method: HttpMethodUi = HttpMethodUi.POST,
        /** Ordered query parameter key/value rows, kept pre-collapse so duplicate-key detection can run
         * against them before they become a `Map`. */
        val queryParamRows: ImmutableList<Pair<String, String>> = persistentListOf(),
    ...
    sealed class UiEvent {
        ...
        data class OnMethodChanged(val method: HttpMethodUi) : UiEvent()
        ...
        data object OnAddQueryParamRow : UiEvent()
        data class OnQueryParamRowKeyChanged(val index: Int, val key: String) : UiEvent()
        data class OnQueryParamRowValueChanged(val index: Int, val value: String) : UiEvent()
        data class OnRemoveQueryParamRow(val index: Int) : UiEvent()
