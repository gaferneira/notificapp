...
fun FilterBottomSheet(
    ...
    CollectOneOffEffects(viewModel.effect) { effect ->
        when (effect) {
            is FilterBottomSheetContract.UiEffect.ApplyFilter -> {
                onFilterApplied(effect.filter)
            }
            is FilterBottomSheetContract.UiEffect.Dismiss -> {
                onDismiss()
            }
        }
    }
    ...
    ModalBottomSheet(
        onDismissRequest = { viewModel.onEvent(FilterBottomSheetContract.UiEvent.OnDismiss) },
...
private fun FilterBottomSheetContent(
    uiState: FilterBottomSheetContract.UiState,
    onEvent: (FilterBottomSheetContract.UiEvent) -> Unit,
) {
    Column(
        modifier = Modifier
            .fillMaxWidth()
            ...
            FlowRow(
                ...
                uiState.availableCategories.forEach { category ->
                    ...
                    FilterChip(
                        selected = isSelected,
                        onClick = {
                            onEvent(FilterBottomSheetContract.UiEvent.OnCategoryToggle(category))
                        },
                        label = { Text(category) },
            ...
            FlowRow(
                ...
                uiState.availableApps.forEach { app ->
                    ...
                    FilterChip(
                        selected = isSelected,
                        onClick = {
                            onEvent(FilterBottomSheetContract.UiEvent.OnAppToggle(app.packageName))
                        },
                        label = { Text(app.name) },
                        colors = FilterChipDefaults.filterChipColors(
                            selectedContainerColor = MaterialTheme.colorScheme.primary,
        ...
        Row(
            ...
            OutlinedButton(
                onClick = { onEvent(FilterBottomSheetContract.UiEvent.OnClearAll) },
            ...
            Button(
                onClick = { onEvent(FilterBottomSheetContract.UiEvent.OnApply) },
                modifier = Modifier.weight(1f),
            ) {
                Icon(
