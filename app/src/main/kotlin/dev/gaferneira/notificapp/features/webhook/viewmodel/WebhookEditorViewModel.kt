...
class WebhookEditorViewModel @Inject constructor(
    ...
    private fun buildWebhook(state: UiState): Webhook {
        ...
        return if (state.id != null) {
            Webhook(id = state.id, name = state.name, url = state.url, headers = headers, auth = auth)
        } else {
            Webhook(name = state.name, url = state.url, headers = headers, auth = auth)
        }
    }

    private fun save() {
        val state = uiState.value
        val duplicateErrors = findDuplicateHeaderKeyErrors(state)
