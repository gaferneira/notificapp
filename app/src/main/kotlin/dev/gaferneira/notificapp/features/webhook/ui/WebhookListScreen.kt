    ...
    Card(
        ...
        Row(
            ...
            Column(modifier = Modifier.weight(1f)) {
                ...
                DeliveryStatusRow(status = webhook.lastDeliveryStatus, lastDeliveryAt = webhook.lastDeliveryAt)
            }
...
/**
 * Tri-state last-delivery indicator (task 5.10): delivered/config-error/unreachable, each with a
 * relative timestamp. Renders nothing for [WebhookDeliveryStatus.UNKNOWN] - a webhook never
 * referenced by a `SEND_WEBHOOK` action has no delivery history to show.
 */
@Composable
private fun DeliveryStatusRow(
    ...
    if (status == WebhookDeliveryStatus.UNKNOWN) return

    val (icon, tint, label) = when (status) {
        WebhookDeliveryStatus.DELIVERED -> Triple(Icons.Default.CheckCircle, MaterialTheme.colorScheme.primary, "Delivered")
        WebhookDeliveryStatus.CONFIG_ERROR -> Triple(Icons.Default.Error, MaterialTheme.colorScheme.error, "Config error")
        WebhookDeliveryStatus.UNREACHABLE -> Triple(Icons.Default.WarningAmber, MaterialTheme.colorScheme.error, "Unreachable")
        WebhookDeliveryStatus.UNKNOWN -> error("Unreachable: guarded above")
    }
    ...
    Row(
        ...
        val timestampSuffix = lastDeliveryAt?.let { " · ${it.toRelativeTimeString()}" }.orEmpty()
        Text(
            text = "$label$timestampSuffix",
            style = MaterialTheme.typography.bodySmall,
            color = tint,
        )
    }
...
/** Relative "x minutes/hours/days ago" rendering for a delivery timestamp. */
private fun Long.toRelativeTimeString(): String = DateUtils.getRelativeTimeSpanString(
    this,
...
@Preview(showBackground = true, device = "id:pixel_5")
@Composable
private fun WebhookListScreenDeliveryStatusPreview() {
    NotificappTheme {
        WebhookListScreenContent(
            uiState = UiState(
                webhooks = persistentListOf(
                    Webhook(
                        ...
                        lastDeliveryStatus = WebhookDeliveryStatus.DELIVERED,
                        lastDeliveryAt = System.currentTimeMillis() - 5 * 60_000,
                    ),
                    Webhook(
                        name = "Slack",
                        url = "https://hooks.slack.com/services/xxx",
                        lastDeliveryStatus = WebhookDeliveryStatus.UNREACHABLE,
                        lastDeliveryAt = System.currentTimeMillis() - 60 * 60_000,
                    ),
                    Webhook(
                        name = "Broken endpoint",
                        url = "https://example.com/hook",
                        lastDeliveryStatus = WebhookDeliveryStatus.CONFIG_ERROR,
                        lastDeliveryAt = System.currentTimeMillis() - 24 * 60 * 60_000,
                    ),
                    Webhook(name = "Never used", url = "https://example.com/unused"),
                ),
                isLoading = false,
            ),
            onEvent = {},
